...
double distanceMSE(int & numPoints, CvPoint2D64f *& imagePoints, double *& imgPointsIn) {
	double dist = 0;
...
void doPOSIT(double cameraMatrix[3 * 3], CvPoint3D64f *& objectPoints, int & numPoints,
		CvPoint2D64f *& imagePoints, int & width, int & height, double rotMatrs[3 * 3],
		double transVects[3]) {
	double shiftI[2];
...
//		memset(cameraMatrix, 0, 9 * sizeof(cameraMatrix[0]));
	...
	imagePoints = (CvPoint2D64f*) cvAlloc(numPoints * sizeof(CvPoint2D64f));

	objectPoints = (CvPoint3D64f*) cvAlloc(numPoints * sizeof(CvPoint3D64f));
